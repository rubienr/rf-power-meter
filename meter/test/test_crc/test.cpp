...
void test_crc_from_string()
{
    ...
    TEST_ASSERT_EQUAL(computeCrc(d0, sizeof(d0) - 1 - strlen("<crc123>")),
                      computeCrc(d1, sizeof(d1) - 1 - strlen("<crc100>")));
}
...
void test_crc_settings_equal()
{
    ...
    TEST_ASSERT_EQUAL(computeCrc(s0.asPtr(), sizeof(s0) - sizeof(s0.crc)),
                      computeCrc(s1.asPtr(), sizeof(s1) - sizeof(s1.crc)));
}
...
void test_crc_settings_not_equal()
{
    ...
    TEST_ASSERT_NOT_EQUAL(computeCrc(s0.asPtr(), sizeof(s0) - sizeof(s0.crc)),
                          computeCrc(s1.asPtr(), sizeof(s1) - sizeof(s1.crc)));
}
